...
class BlockDetectionHandler:
    ...
    def on_blk_detect_complete(self, result): 
        ...
        # Transfer IDs from previous blocks to new blocks based on proximity
        # This ensures that re-running OCR preserves existing block IDs and UUIDs
        for new_blk in self.main_page.blk_list:
            best_match = None
            min_dist = 50 # Maximum pixels apart to be considered the same bubble
            
            n_cx = (new_blk.xyxy[0] + new_blk.xyxy[2]) / 2
            n_cy = (new_blk.xyxy[1] + new_blk.xyxy[3]) / 2
            
            for prev_blk in previous_blocks:
                p_cx = (prev_blk.xyxy[0] + prev_blk.xyxy[2]) / 2
                p_cy = (prev_blk.xyxy[1] + prev_blk.xyxy[3]) / 2
                
                dist = ((n_cx - p_cx)**2 + (n_cy - p_cy)**2)**0.5
                if dist < min_dist:
                    min_dist = dist
                    best_match = prev_blk
            
            if best_match:
                new_blk.block_id = best_match.block_id
                new_blk.uuid = best_match.uuid
