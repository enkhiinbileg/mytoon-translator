...
class TranslationHandler:
    ...
    def translate_image(self, single_block=False):
        ...
        if self.main_page.image_viewer.hasPhoto() and self.main_page.blk_list:
            ...
            if translator_key == "None (Raw Text)":
                blocks_to_process = [self.pipeline.get_selected_block()] if single_block else self.main_page.blk_list
                for blk in blocks_to_process:
                    if blk is not None:
                        blk.translation = blk.text
                set_upper_case(blocks_to_process, upper_case)
                logger.info("Translation bypassed. Using raw text.")
                return
    ...
    def translate_webtoon_visible_area(self, single_block=False):
        ...
        if translator_key == "None (Raw Text)":
            for blk in visible_blocks:
                blk.translation = blk.text
            logger.info("Translation bypassed in webtoon mode. Using raw text.")
        else:
            translator = Translator(self.main_page, source_lang, target_lang)
