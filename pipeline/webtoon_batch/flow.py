...
class FlowMixin:
    ...
    def _emit_progress(
        ...
        # Scale step to cover the fractional part of the physical page
        # total_steps in the signal is 10 * total_virtual
        scaled_step = virtual_index * 10 + step
        scaled_total_steps = 10 * total_virtual
        self.main_page.progress_update.emit(index, total_images, scaled_step, scaled_total_steps, change_name)
    ...
    def _load_virtual_record(
        ...
        if emit_progress:
            self._emit_progress(
                ...
                1, 
                False,
                virtual_index=vpage.virtual_index,
                total_virtual=record.get("total_virtual", 1)
            )
        return record
        ...
        try:
            ...
            for index, record in enumerate(virtual_records):
                ...
                if (
                    ...
                    cached_current = None
                    self._emit_progress(
                        current_record["selected_index"], 
                        total_images, 
                        1, 
                        False,
                        virtual_index=current_record["vpage"].virtual_index,
                ...
                if self._can_pair_for_seam(current_record, next_record):
                    self._emit_progress(
                        ...
                        3, 
                        False,
                        virtual_index=current_vpage.virtual_index,
                        total_virtual=current_record.get("total_virtual", 1)
                    )
