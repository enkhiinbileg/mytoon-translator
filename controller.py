...
class ComicTranslate(ComicTranslateUI):
    ...
    def __init__(self, parent=None):
        ...
        # Expose main controller to image viewer for data access
        self.image_viewer.main = self
        ...
        # Periodic Auth Check (every 10 minutes)
        from modules.auth_manager import AuthManager
        self.auth_manager = AuthManager()
        self.auth_check_timer = QtCore.QTimer(self)
        self.auth_check_timer.setInterval(10 * 60 * 1000) # 10 minutes
        self.auth_check_timer.timeout.connect(self.periodic_auth_check)
        self.auth_check_timer.start()

    def periodic_auth_check(self):
        if not self.auth_manager.check_access():
            from app.ui.messages import Messages
            Messages.show_error(self, self.tr("Access Revoked"), self.tr("Таны ашиглах эрх цуцлагдсан байна. Программ хаагдах болно."))
            self.close()
    ...
    def connect_ui_elements(self):
        ...
        # Magic Wand now handled via tool toggle and tap signal
        self.workspace_magic_wand_all_button.clicked.connect(self.batch_text_ctrl.magic_wand_all)
        ...
        self.image_viewer.rect_clean_requested.connect(self.batch_text_ctrl.solid_fill_clean)
        self.image_viewer.inpaint_rect_requested.connect(self.batch_text_ctrl.inpaint_rect_clean)
        self.patches_processed.connect(self.image_ctrl.on_inpaint_patches_processed)
        ...
        self.paste_all_button.clicked.connect(self.batch_text_ctrl.import_all_text)
        self.workspace_copy_all_button.clicked.connect(self.batch_text_ctrl.export_all_text)
    ...
    def on_manual_finished(self):
        ...
        self.enable_hbutton_group()
        # Ensure translate button is enabled
        self.translate_button.setEnabled(True)
    ...
    def enable_hbutton_group(self):
        ...
        # Only enable translate_button if not in manual mode, or just always allow it now
        self.translate_button.setEnabled(True)
