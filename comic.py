    ...
    class LoadingCoordinator(QObject):
        ...
        def on_finished(self, project_file):
            try:
                ...
                auth = AuthManager()
                saved_email = auth.get_saved_email()
                print(f"DEBUG: Startup check. Saved email: '{saved_email}'")
                
                authorized = False
                if saved_email:
                    authorized = auth.check_access(saved_email)
                    print(f"DEBUG: Saved email authorized: {authorized}")
                    if not authorized:
                        auth.logout() # Clear invalid email

                if not authorized:
                    print("DEBUG: Showing LoginDialog because authorized is False")
                    splash.hide() # Hide splash while login is shown
                    login = LoginDialog(saved_email)
                    
                    def perform_login(email):
                        if auth.check_access(email):
                            auth.save_email(email)
                            login.accept()
                        else:
                            from app.ui.messages import Messages
                            Messages.show_error(login, "Эрхгүй", "Танд ашиглах эрх байхгүй байна!")

                    login.login_requested.connect(perform_login)
                    if login.exec() != LoginDialog.Accepted:
                        splash.close()
                        app.quit()
                        return

                # 2. Proceed with loading the UI
