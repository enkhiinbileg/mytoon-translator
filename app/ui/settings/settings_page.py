...
from app.account.auth.auth_client import AuthClient, USER_INFO_GROUP, EMAIL_KEY, TIER_KEY, CREDITS_KEY, MONTHLY_CREDITS_KEY
...
class SettingsPage(QtWidgets.QWidget):
    ...
    def _is_online(self) -> bool:
        ...
        if self.auth_manager.get_saved_email():
             # Just update the view based on saved email
    ...
    def _load_user_info_from_settings(self):
        ...
        self.user_email = self.auth_manager.get_saved_email()
        ...
        raw_credits = settings.value(CREDITS_KEY, None)
        if isinstance(raw_credits, str) and raw_credits.startswith('{'):
            try:
                self.user_credits = json.loads(raw_credits)
            except Exception:
                self.user_credits = raw_credits
        else:
            self.user_credits = raw_credits
        settings.endGroup()
    ...
    def start_sign_in(self):
        """Initiates the real authentication flow."""
        logger.info("Sign In button clicked. Starting real auth flow.")
        
        # Update button state to show loading/progress
        self.ui.sign_in_button.setText(self.tr("Connecting..."))
        self.ui.sign_in_button.setEnabled(False)
        
        # Disconnect previous connection and connect to cancel
        try:
    ...
    def handle_auth_success(self, user_info: dict):
        ...
        if self.user_email:
            self.auth_manager.save_email(self.user_email)
    ...
    def is_logged_in(self) -> bool:
        ...
        return bool(self.user_email) or bool(self.auth_manager.get_saved_email())
