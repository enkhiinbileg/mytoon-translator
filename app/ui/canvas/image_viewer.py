...
class ImageViewer(QGraphicsView):
    ...
    # Properties to maintain public API
    @property
    def webtoon_mode(self) -> bool:
        """Read-only proxy to check if webtoon mode is active."""
        if hasattr(self, 'webtoon_manager') and self.webtoon_manager:
    ...
    def hasPhoto(self) -> bool:
        if self.webtoon_mode:
            # In webtoon mode, we have a "photo" if we have images loaded in the manager
            return len(self.webtoon_manager.image_loader.image_file_paths) > 0
        return not self.empty and self.photo.pixmap() is not None
        ...
        try:
            ...
            if self.webtoon_mode:
                if not self.empty:
                    ...
                    image_rect = QRectF()
                    if self.webtoon_manager.image_items:
                        first_item = None
                    ...
                    if not image_rect.isNull() and image_rect.width() > 0 and image_rect.height() > 0:
                        padding = 20
                        padded_rect = image_rect.adjusted(-padding, -padding, padding, padding)
                        
                        # Ensure we have the full scene rect before scaling to avoid scroll limits
                        full_width = self.webtoon_manager.webtoon_width
                        full_height = self.webtoon_manager.total_height
                        self.setSceneRect(0, 0, full_width, full_height)
                        
                        unity = self.transform().mapRect(QRectF(0, 0, 1, 1))
                        if unity.width() > 0.0001 and unity.height() > 0.0001:
                            self.scale(1 / unity.width(), 1 / unity.height())
                        
                        viewrect = self.viewport().rect()
                        scenerect = self.transform().mapRect(padded_rect)
                        if scenerect.width() > 0.0001 and scenerect.height() > 0.0001:
    ...
    def add_text_item(self, properties) -> TextBlockItem:
        ...
        item.uuid = properties.uuid
        item.block_id = properties.block_id
        item.target_lang = properties.target_lang
        item.source_lang = properties.source_lang
    ...
    def load_state(self, state: Dict):
        ...
        for i, data in enumerate(state.get('text_items_state', [])):
            # Use the new add_text_item function for consistency
            item = self.add_text_item(data)
            # Tag with Stable ID (block_id) or UUID
            blk_uuid = data.get('uuid')
            blk_id = data.get('block_id', -1)
            item.uuid = blk_uuid
            item.block_id = blk_id
            # UserRole storage: try block_id first if valid (>0), then uuid, then index
            item.setData(QtCore.Qt.UserRole, blk_id if blk_id >= 0 else (blk_uuid if blk_uuid else i))
