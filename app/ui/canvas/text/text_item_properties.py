...
class TextItemProperties:
    ...
    def from_dict(cls, data: dict) -> 'TextItemProperties':
        ...
        props.uuid = data.get('uuid')
        props.block_id = data.get('block_id', -1)
        props.target_lang = data.get('target_lang')
        props.source_lang = data.get('source_lang')
    ...
    def from_text_item(cls, item) -> 'TextItemProperties':
        ...
        props.uuid = getattr(item, 'uuid', None)
        props.block_id = getattr(item, 'block_id', -1)
        props.target_lang = getattr(item, 'target_lang', None)
        props.source_lang = getattr(item, 'source_lang', None)
