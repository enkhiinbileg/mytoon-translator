...
class EventHandler:
        ...
        if event.button() == Qt.LeftButton:
            ...
            if self.viewer.current_tool == 'rect_clean':
                if self._is_on_image(scene_pos):
                    ...
                    color = self._get_color_at_pos(scene_pos)
                    self.viewer.start_point = scene_pos
                    self.viewer._picked_color = color
                    # Create a preview rect
                    self.viewer.current_rect = self.viewer.create_rect_item(QtCore.QRectF(0, 0, 0, 0))
                    self.viewer.current_rect.setPos(scene_pos)
                    # Set color preview with dashed contrast border
                    fill_color = QtGui.QColor(color)
                    fill_color.setAlpha(128) # Semi-transparent preview
                    self.viewer.current_rect.setBrush(QtGui.QBrush(fill_color))
                    
                    # Create a dashed pen that is visible on any background
                    pen = QtGui.QPen(QtGui.QColor(255, 255, 255), 1)
                    pen.setStyle(QtCore.Qt.PenStyle.DashLine)
                    self.viewer.current_rect.setPen(pen)
                    return

            # Inpaint Rect Tool - Start selection
            if self.viewer.current_tool == 'inpaint_rect':
                if self._is_on_image(scene_pos):
                    self.viewer.start_point = scene_pos
                    # Create a preview rect with a distinct style (e.g., magenta)
                    self.viewer.current_rect = self.viewer.create_rect_item(QtCore.QRectF(0, 0, 0, 0))
                    self.viewer.current_rect.setPos(scene_pos)
                    pen = QtGui.QPen(QtGui.QColor(255, 0, 255), 1)
                    pen.setStyle(QtCore.Qt.PenStyle.DashLine)
                    self.viewer.current_rect.setPen(pen)
                    # Semi-transparent magenta fill for preview
                    fill = QtGui.QColor(255, 0, 255, 64)
                    self.viewer.current_rect.setBrush(QtGui.QBrush(fill))
                    return
    ...
    def handle_mouse_move(self, event: QtGui.QMouseEvent):
        ...
        # Then handle other interactions like resize/rotate hover, but skip if we're in a drawing/selection tool
        if self.viewer.current_tool not in ['brush', 'eraser', 'rect_clean', 'inpaint_rect']:
            if self._move_handle_item_interaction(scene_pos): 
                return
        else:
            # Enforce the tool's cursor if we're in a drawing/selection tool
            if self.viewer.current_tool in ['rect_clean', 'inpaint_rect']:
                self.viewer.viewport().setCursor(Qt.CursorShape.CrossCursor)
            elif self.viewer.current_tool == 'brush':
                self.viewer.viewport().setCursor(self.viewer.drawing_manager.brush_cursor)
            elif self.viewer.current_tool == 'eraser':
                self.viewer.viewport().setCursor(self.viewer.drawing_manager.eraser_cursor)
        ...
        if self.viewer.current_tool in ['box', 'rect_clean', 'inpaint_rect']:
    ...
    def handle_wheel(self, event: QtGui.QWheelEvent):
        ...
        if event.modifiers() == Qt.KeyboardModifier.ControlModifier:
            ...
            self.viewer.scale_changed.emit(self.viewer.transform().m11())
        else:
    ...
    def handle_viewport_event(self, event: QtCore.QEvent):
        if event.type() in (QEvent.Type.TouchBegin, QEvent.Type.TouchUpdate, QEvent.Type.TouchEnd):
            ...
            if len(touch_points) == 2:
                ...
                self.viewer.scale_changed.emit(scale_factor)
                return True
    ...
    def _get_color_at_pos(self, scene_pos: QPointF) -> QtGui.QColor:
        """Samples the color of the underlying image at the given scene position."""
        if self.viewer.webtoon_mode:
            manager = self.viewer.webtoon_manager
            page_idx = manager.layout_manager.get_page_at_position(scene_pos.y())
            if page_idx >= 0:
                # Map scene to page local
                local_pos = manager.coordinate_converter.scene_to_page_local_position(scene_pos, page_idx)
                if local_pos:
                    # Access data through main controller
                    main = getattr(self.viewer, 'main', None)
                    if main and page_idx < len(main.image_files):
                        file_path = main.image_files[page_idx]
                        img = main.image_data.get(file_path)
                        if img is not None:
                            h, w = img.shape[:2]
                            ix, iy = int(local_pos.x()), int(local_pos.y())
                            if 0 <= ix < w and 0 <= iy < h:
                                c = img[iy, ix]
                                return QtGui.QColor(int(c[0]), int(c[1]), int(c[2]))
        elif self.viewer.hasPhoto():
            pixmap = self.viewer.photo.pixmap()
            if not pixmap.isNull():
                img = pixmap.toImage()
                local_pos = self.viewer.photo.mapFromScene(scene_pos)
                ix, iy = int(local_pos.x()), int(local_pos.y())
                if 0 <= ix < img.width() and 0 <= iy < img.height():
                    return QtGui.QColor(img.pixelColor(ix, iy))
    ...
    def _release_handle_rect_clean(self):
        if self.viewer.current_rect:
            rect = self.viewer.current_rect.rect()
            if rect.width() > 0 and rect.height() > 0:
                scene_rect = self.viewer.current_rect.mapRectToScene(rect)
                color = getattr(self.viewer, '_picked_color', QtGui.QColor(255, 255, 255))
                self.viewer.rect_clean_requested.emit(scene_rect, color)
            self.viewer._scene.removeItem(self.viewer.current_rect)
        self.viewer.current_rect = None
        self.viewer._picked_color = None

    def _release_handle_inpaint_rect(self):
        if self.viewer.current_rect:
            rect = self.viewer.current_rect.rect()
            if rect.width() > 0 and rect.height() > 0:
                scene_rect = self.viewer.current_rect.mapRectToScene(rect)
