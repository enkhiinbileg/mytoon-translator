...
class TextItemManager:
    ...
    def load_text_items(self, state: dict, page_idx: int):
        ...
        for i, text_data in enumerate(text_items):
            ...
            blk_uuid = text_data.get('uuid')
            blk_id = text_data.get('block_id', -1)
            text_item.uuid = blk_uuid
            text_item.block_id = blk_id
            text_item.setData(Qt.UserRole, blk_id if blk_id >= 0 else (blk_uuid if blk_uuid else i))
