...
class LazyImageLoader:
    ...
    def _initial_load(self, current_page: int):
        ...
        self.load_timer.start(10)  # Faster processing (10ms)
    ...
    def _process_load_queue(self):
        ...
        # Load up to 2 pages per timer tick for faster sequential appearance
        for _ in range(2):
            if not self.load_queue:
                break
    ...
    def _load_single_page(self, page_idx: int):
        """Load a single page's image and content using a background thread."""
        if (
        ...
        def _on_loaded(img):
            try:
                if img is not None:
                    self._process_loaded_image(page_idx, img)
            finally:
                self.loading_pages.discard(page_idx)
                # Check if we need to manage memory after a load
                self._manage_memory()

        # Use viewer's thread pool to load the image
        if hasattr(self.viewer, 'run_threaded'):
            self.viewer.run_threaded(_bg_read, _on_loaded)
        else:
            # Fallback if run_threaded is not available
            img = _bg_read()
            _on_loaded(img)

    def _process_loaded_image(self, page_idx: int, img: np.ndarray):
        """Handle the loaded image data on the main thread."""
        try:
            file_path = self.image_file_paths[page_idx]
            # Convert to QPixmap
            h, w, c = img.shape
            ...
            import traceback
            print(f"Error processing loaded image for page {page_idx}: {e}")
            traceback.print_exc()
