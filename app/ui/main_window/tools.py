...
class ToolStateMixin:
    ...
    def toggle_magic_wand_tool(self):
        if self.workspace_magic_wand_button.isChecked():
            self.set_tool("magic_wand")
        else:
            self.set_tool(None)

    def toggle_rect_clean_tool(self):
        if self.workspace_rect_clean_button.isChecked():
            self.set_tool("rect_clean")
        else:
            self.set_tool(None)

    def toggle_inpaint_rect_tool(self):
        if self.workspace_inpaint_rect_button.isChecked():
            self.set_tool("inpaint_rect")
        else:
            self.set_tool(None)
    ...
    def set_tool(self, tool_name: str):
        ...
        if not tool_name:
            ...
            self.image_viewer.unsetCursor()
        elif tool_name in ["rect_clean", "inpaint_rect", "box"]:
            self.image_viewer.setCursor(QtCore.Qt.CursorShape.CrossCursor)
        elif tool_name == "pan":
            self.image_viewer.setCursor(QtCore.Qt.CursorShape.OpenHandCursor)
        else:
            self.image_viewer.unsetCursor()
