...
from PySide6.QtCore import Signal, Qt, QSize, QRect, QRectF, QEvent
from PySide6.QtGui import QContextMenuEvent, QDropEvent, QPainter, QPixmap, QFont, QColor, QPainterPath
...
class PageListItemDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option, index):
        ...
        # Draw rounded background for selection or hover
        ...
        # Use white for selected text, or primary text color for normal
        pen_color = QColor("#ffffff") if selected else QColor(dayu_theme.primary_text_color)
    ...
    def _background_color(self, option) -> QColor:
        if option.state & QStyle.StateFlag.State_Selected:
            return QColor(dayu_theme.primary_color)
        if option.state & QStyle.StateFlag.State_MouseOver:
            # Subtle hover blend
            base = QColor(dayu_theme.background_color)
            accent = QColor(dayu_theme.primary_color)
            return self._blend_colors(base, accent, 0.15)
        return QColor(dayu_theme.background_color)
