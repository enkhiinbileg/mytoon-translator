...
class ImageStateController:
    ...
    def load_initial_image(self, file_paths: List[str]):
        ...
        file_paths.sort(key=lambda x: self._natural_sort_key(os.path.basename(x)))
        self.main.image_files = file_paths

        # Initialize states for all files before loading anything to avoid KeyErrors
        for file_path in file_paths:
            if file_path not in self.main.image_states:
                self.main.image_states[file_path] = self._build_image_state(file_path, {}, [], [], False)
        ...
        if self.main.image_files:
            def on_files_prepared(prepared_files):
                if not prepared_files:
                    return
                # Save current path to restore index after sorting
                current_path = self._current_file_path()

                # Add new files
                self.main.image_files.extend(prepared_files)
                # Natural sort by filename
                self.main.image_files.sort(key=lambda x: self._natural_sort_key(os.path.basename(x)))

                # Ensure we are in Webtoon mode when adding files
                self.main.webtoon_toggle.setChecked(True)
                self.main.webtoon_mode = True

                # Restore current index
                if current_path in self.main.image_files:
                    self.main.curr_img_idx = self.main.image_files.index(current_path)
                ...
                # Refresh the view and update UI safely on the main thread
                def _update_ui_after_insert():
                    self.refresh_page_list()
                    if self.main.webtoon_mode:
                        # Refresh lazy loader if already in webtoon mode
                        ...
                        # Jump to the same page in regular mode
                        new_idx = max(0, self.main.curr_img_idx)
                        self.main.page_list.setCurrentRow(new_idx)
                        path = self.main.image_files[new_idx]
                        im = self.load_image(path)
                        self.display_image_from_loaded(im, new_idx, False)
                
                QtCore.QTimer.singleShot(0, _update_ui_after_insert)
                self.main.mark_project_dirty()
    ...
    def on_initial_image_loaded(self, rgb_image: np.ndarray):
        ...
        if self.main.image_files:
            ...
            # Default to regular mode for new sets of images on the main thread
            ...
            # If we were previously in webtoon mode, we might need to reset the viewer
            if self.main.webtoon_mode:
                self.main.webtoon_ctrl.switch_to_regular_mode()
    ...
    def refresh_page_list(self):
        ...
        try:
            ...
            for idx, file_path in enumerate(self.main.image_files):
                ...
                display_name = f"{idx + 1}. {file_name}"
    ...
    def on_page_list_current_item_changed(self, current, previous):
        ...
        if self.main.webtoon_mode:
            ...
            if self.main.image_viewer.hasPhoto():
                ...
                self.main.compare_ctrl.sync_content()
            else:
    ...
    def display_image(self, index: int, switch_page: bool = True):
        if 0 <= index < len(self.main.image_files):
            ...
            if not self.main.compare_ctrl.is_active:
                if self.main.webtoon_mode:
                ...
                # In compare mode, just scroll if in webtoon
                if self.main.webtoon_mode:
                    self.main.image_viewer.scroll_to_page(index)
    ...
    def on_inpaint_patches_processed(self, patches: list, file_path: str):
        ...
        print(f"DEBUG: Processing {len(patches)} patches for {file_path}")
        ...
        if self.main.image_viewer.webtoon_mode:
            ...
            print(f"DEBUG: Webtoon mode. Page index: {page_index}, Loaded pages: {loaded_pages}")
        ...
        print(f"DEBUG: should_display for {file_path} is {should_display}")
